...
class TarefaIT extends IntegrationTestBase {
  ...
  record TarefaReq(
      String titulo, String descricao, String prazo,
      String prioridade, UUID responsavelId,
      String entidadeTipo, UUID entidadeId) {}

  record TarefaResp(UUID id, String titulo, String status, String prioridade,
      UUID responsavelId, String concluidaEm) {}
  ...
  void setup() {
    ...
    token = rest.postForEntity(
        ...
        new SignupReq("Tarefa IT A", "tar-it-a-" + ts, "ta" + ts + "@test.com", "senha123456"),
        TokenResp.class).getBody().accessToken();

    token2 = rest.postForEntity(
        baseUrl("/api/v1/auth/signup"),
        new SignupReq("Tarefa IT B", "tar-it-b-" + ts, "tb" + ts + "@test.com", "senha123456"),
        TokenResp.class).getBody().accessToken();
  }
  ...
  TarefaResp criar(String titulo, String prazo, String tok) {
    ...
    return rest.exchange(
        baseUrl("/api/v1/tarefas"), HttpMethod.POST,
        new HttpEntity<>(req, h(tok)), TarefaResp.class).getBody();
  }
  ...
  void ac1_criar_sem_titulo_retorna_400() {
    var req = new TarefaReq(null, null, Instant.now().plus(1, ChronoUnit.DAYS).toString(),
        "MEDIA", null, null, null);
    ResponseEntity<Map> resp = rest.exchange(
        baseUrl("/api/v1/tarefas"), HttpMethod.POST,
        new HttpEntity<>(req, h(token)), Map.class);
  ...
  void ac3_marcar_feita_registra_concluida_em() {
    ...
    ResponseEntity<TarefaResp> resp = rest.exchange(
        baseUrl("/api/v1/tarefas/" + t.id() + "/status"), HttpMethod.PATCH,
        new HttpEntity<>(new StatusReq("FEITA"), h(token)), TarefaResp.class);
  ...
  void ac3_reabrir_limpa_concluida_em() {
    ...
    ResponseEntity<TarefaResp> resp = rest.exchange(
        baseUrl("/api/v1/tarefas/" + t.id() + "/status"), HttpMethod.PATCH,
        new HttpEntity<>(new StatusReq("TODO"), h(token)), TarefaResp.class);
  ...
  void ac5_cross_tenant_retorna_404() {
    ...
    ResponseEntity<Map> resp = rest.exchange(
        baseUrl("/api/v1/tarefas/" + t.id()), HttpMethod.GET,
        new HttpEntity<>(h(token2)), Map.class);
  ...
  void ac5_cross_tenant_delete_retorna_404() {
    ...
    ResponseEntity<Map> resp = rest.exchange(
        baseUrl("/api/v1/tarefas/" + t.id()), HttpMethod.DELETE,
        new HttpEntity<>(h(token2)), Map.class);
    assertThat(resp.getStatusCode().value()).isEqualTo(404);
  }
  ...
  void ac11_comentario_criado_e_listado() {
    ...
    ResponseEntity<ComentarioResp> c = rest.exchange(
        baseUrl("/api/v1/tarefas/" + t.id() + "/comentarios"), HttpMethod.POST,
        new HttpEntity<>(new ComentarioReq("Ligar para o cliente amanhã"), h(token)),
    ...
    ResponseEntity<List<ComentarioResp>> lista = rest.exchange(
        baseUrl("/api/v1/tarefas/" + t.id() + "/comentarios"), HttpMethod.GET,
  ...
  void alerta_inclui_tarefas_vencidas() {
    ...
    ResponseEntity<Map> resp = rest.exchange(
        baseUrl("/api/v1/tarefas/alerta"), HttpMethod.GET,
        new HttpEntity<>(h(token)), Map.class);
    assertThat(resp.getStatusCode().value()).isEqualTo(200);
  ...
  void preferencias_default_digest_enabled() {
    ResponseEntity<Map> resp = rest.exchange(
        baseUrl("/api/v1/tarefas/preferencias"), HttpMethod.GET,
        new HttpEntity<>(h(token)), Map.class);
    assertThat(resp.getStatusCode().value()).isEqualTo(200);
  ...
  void preferencias_optout_digest() {
    rest.exchange(
        baseUrl("/api/v1/tarefas/preferencias"), HttpMethod.PATCH,
        new HttpEntity<>(Map.of("digestDiarioEnabled", false), h(token)), Map.class);
