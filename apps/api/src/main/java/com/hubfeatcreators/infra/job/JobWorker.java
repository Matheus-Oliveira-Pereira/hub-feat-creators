  ...
  public void poll() {
    ...
    for (Job job : jobs) {
      ...
      try {
        ...
        log.warn("job.fail tipo={} id={} tentativa={}", job.getTipo(), job.getId(), job.getTentativas(), e);
        if (job.getTentativas() >= job.getMaxTentativas()) {
          fail(job, e.getMessage());
        } else {
          long backoff = BACKOFF_MINUTES[Math.min(job.getTentativas() - 1, BACKOFF_MINUTES.length - 1)];
