...
public class ContatoController {
  ...
  record ContatoResponse(
      UUID id, UUID marcaId, String nome, String email, String telefone, String cargo,
      boolean emailInvalido, Instant createdAt, Instant updatedAt) {}

  @PostMapping
  @ResponseStatus(HttpStatus.CREATED)
  public ContatoResponse criar(
      @Valid @RequestBody ContatoRequest req, @AuthenticationPrincipal AuthPrincipal principal) {
  ...
  private ContatoResponse toResponse(Contato c) {
    return new ContatoResponse(c.getId(), c.getMarcaId(), c.getNome(), c.getEmail(),
        c.getTelefone(), c.getCargo(), c.isEmailInvalido(), c.getCreatedAt(), c.getUpdatedAt());
  }
}
