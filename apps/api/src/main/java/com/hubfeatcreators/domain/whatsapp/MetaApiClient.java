    ...
    public String submitTemplate(
        ...
        try {
            JsonNode resp =
                    rest.post()
                            .uri(
                                    "/{wabaId}/message_templates?access_token={token}",
                                    wabaId,
                                    accessToken)
            ...
            if (resp == null || !resp.has("id"))
                throw new MetaApiException("Meta não retornou id do template");
            return resp.get("id").asText();
        } catch (HttpClientErrorException e) {
            throw new MetaApiException(
                    "Erro ao submeter template: " + e.getResponseBodyAsString(), e);
        }
    }
    ...
    public String getTemplateStatus(String wabaId, String accessToken, String metaTemplateId) {
        try {
            JsonNode resp =
                    rest.get()
                            .uri(
                                    "/{wabaId}/message_templates?name={id}&access_token={token}",
                                    wabaId,
                                    metaTemplateId,
                                    accessToken)
                            .retrieve()
    ...
    private String doSend(String phoneNumberId, String accessToken, Map<String, Object> body) {
        try {
            JsonNode resp =
                    rest.post()
                            .uri(
                                    "/{phoneId}/messages?access_token={token}",
                                    phoneNumberId,
                                    accessToken)
                            .contentType(MediaType.APPLICATION_JSON)
                            .body(body)
            ...
            throw new MetaApiException(
                    "Erro ao enviar mensagem WA: " + e.getResponseBodyAsString(), e);
        }
    }
}
