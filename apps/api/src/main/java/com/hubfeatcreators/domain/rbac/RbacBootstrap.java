...
/**
 * Cria os perfis seed (Owner, Assessor, Leitor) ao criar uma assessoria nova,
 * e atribui o perfil correto ao usuário recém-criado conforme {@link Usuario#getRole()}.
 *
...
public class RbacBootstrap {
  ...
  public Perfil seedAssessoria(UUID assessoriaId) {
    Perfil owner = perfilRepo.save(
        new Perfil(assessoriaId, "Owner", "Acesso total à assessoria",
            PermissionCodes.OWNER_DEFAULT, true));
    perfilRepo.save(
        new Perfil(assessoriaId, "Assessor", "Operacional padrão de prospecção e cadastros",
            PermissionCodes.ASSESSOR_DEFAULT, true));
    perfilRepo.save(
        new Perfil(assessoriaId, "Leitor", "Somente leitura",
            PermissionCodes.LEITOR_DEFAULT, true));
    return owner;
  }
    ...
    return perfilRepo
        ...
        .orElseThrow(
            () -> new IllegalStateException(
                "Perfil seed '" + nome + "' não encontrado para assessoria " + assessoriaId));
  }
}
