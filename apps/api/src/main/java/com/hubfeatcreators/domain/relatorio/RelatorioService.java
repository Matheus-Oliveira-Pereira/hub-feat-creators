...
public class RelatorioService {
    ...
    public record AssessorItem(
            ...
            long abertasAnterior,
            long ganhasAnterior,
            long perdidasAnterior) {}

    public record PerformanceResult(
            List<AssessorItem> assessores, String periodoAtual, String periodoAnterior) {}
    ...
    public record TarefaSlaItem(
            ...
            long noPrazo,
            long atrasadas,
            double atrasoMedioHoras,
            long totalAnterior) {}

    public record TarefaSlaResult(
            List<TarefaSlaItem> assessores, String periodoAtual, String periodoAnterior) {}
    ...
    public FunilResult funil(UUID assessoriaId, Instant from, Instant to, UUID assessorId) {
        ...
        List<String> allStatuses =
                List.of("NOVA", "CONTATADA", "NEGOCIANDO", "FECHADA_GANHA", "FECHADA_PERDIDA");
        ...
        for (String status : allStatuses) {
            ...
            double delta =
                    qtdAnterior == 0
                            ? (qtd > 0 ? 100.0 : 0.0)
                            : ((qtd - qtdAnterior) * 100.0 / qtdAnterior);
        ...
        for (Map<String, Object> row : current) {
            ...
            assessores.add(
                    new AssessorItem(
                            aId.toString(),
                            email,
                            toLong(row.get("abertas")),
                            toLong(row.get("ganhas")),
                            toLong(row.get("perdidas")),
                            toLong(row.get("ticket_medio_centavos")),
                            toDouble(row.get("tempo_medio_fechamento_dias")),
        ...
        for (Map<String, Object> row : current) {
            ...
            String email =
                    usuarios.stream()
                            ...
                            .map(u -> (String) u.get("email"))
                            .findFirst()
                            .orElse(rId.toString().substring(0, 8));
            ...
            assessores.add(
                    new TarefaSlaItem(
                            ...
                            toLong(row.get("no_prazo")),
                            toLong(row.get("atrasadas")),
                            toDouble(row.get("atraso_medio_horas")),
                            totalAnterior));
        }
    ...
    public void exportPerformanceCsv(UUID assessoriaId, Instant from, Instant to, PrintWriter out) {
        ...
        r.assessores()
                .forEach(
                        i ->
                                out.printf(
                                        "%s,%s,%d,%d,%d,%d,%.2f,%d,%d,%d%n",
                                        i.assessorId(),
                                        i.assessorEmail(),
                                        i.abertas(),
                                        i.ganhas(),
                                        i.perdidas(),
                                        i.ticketMedioCentavos(),
                                        i.tempoMedioFechamentoDias(),
                                        i.abertasAnterior(),
                                        i.ganhasAnterior(),
                                        i.perdidasAnterior()));
    }
    ...
    private void validate(Instant from, Instant to) {
        if (from.isAfter(to)) {
            throw BusinessException.badRequest(
                    "RELATORIO_PERIODO_INVALIDO", "'from' deve ser anterior a 'to'.");
        }
        long months =
                ChronoUnit.MONTHS.between(
                        from.atZone(BRT).toLocalDate(), to.atZone(BRT).toLocalDate());
        if (months > MAX_MONTHS) {
            throw BusinessException.badRequest(
                    "RELATORIO_PERIODO_MUITO_LONGO", "Período máximo é 24 meses.");
        }
    }
