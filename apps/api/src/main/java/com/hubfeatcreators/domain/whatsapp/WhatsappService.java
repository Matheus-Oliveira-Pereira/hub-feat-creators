...
public class WhatsappService {
    ...
    public WhatsappService(
            WhatsappEnvioRepository envioRepo,
        ...
        envioRepo
                ...
                .ifPresent(
                        envio -> {
                            ...
                            switch (status) {
                                case "delivered" -> {
                                    envio.setStatus("ENTREGUE");
                                    envio.setDeliveredAt(now);
                                }
                                case "read" -> {
                                    envio.setStatus("LIDO");
                                    envio.setReadAt(now);
                                }
                                case "failed" -> {
                                    envio.setStatus("FALHOU");
                                    envio.setFailedAt(now);
                                }
                                default -> {
    ...
    private String sendTemplateViaMeta(WhatsappEnvio envio, WhatsappAccount account, String token) {
        try {
            ...
            return meta.sendTemplate(
                    account.getPhoneNumberId(),
                    token,
                    envio.getDestinatarioE164(),
                    templateNome,
                    idioma,
                    components);
    ...
    private String sendFreeformViaMeta(WhatsappEnvio envio, WhatsappAccount account, String token) {
        try {
            ...
            return meta.sendText(
                    account.getPhoneNumberId(), token, envio.getDestinatarioE164(), text);
        } catch (JsonProcessingException e) {
            throw new IllegalStateException("Payload inválido", e);
        }
    }
