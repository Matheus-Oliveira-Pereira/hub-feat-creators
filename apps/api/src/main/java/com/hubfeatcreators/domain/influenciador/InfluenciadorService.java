...
public class InfluenciadorService {
  ...
  public Influenciador criar(AuthPrincipal principal, InfluenciadorRequest req) {
    Influenciador inf = new Influenciador(principal.assessoriaId(), req.nome(), principal.usuarioId());
    applyRequest(inf, req);
    inf = repo.save(inf);

    auditLogService.log(
        principal.assessoriaId(), principal.usuarioId(), "influenciador", inf.getId(),
        AuditLog.Acao.CREATE, toMap(inf));

    jobService.enqueue(
        principal.assessoriaId(), JOB_LGPD,
        Map.of("influenciadorId", inf.getId().toString(), "email", req.handles().getOrDefault("email", "")),
        inf.getId());
  ...
  public Influenciador atualizar(AuthPrincipal principal, UUID id, InfluenciadorRequest req) {
    ...
    auditLogService.log(
        principal.assessoriaId(), principal.usuarioId(), "influenciador", id,
        AuditLog.Acao.UPDATE, toMap(inf));
