...
public class InfluenciadorController {
  ...
  public PageResponse<InfluenciadorResponse> listar(
    ...
    List<InfluenciadorResponse> data = result.getContent().stream().map(this::toResponse).toList();
          ...
          for (Influenciador inf : all) {
            String line =
                String.format(
                    ...
                    inf.getAudienciaTotal() != null ? inf.getAudienciaTotal() : "",
  ...
  private InfluenciadorResponse toResponse(Influenciador inf) {
    return new InfluenciadorResponse(
        inf.getId(), inf.getNome(), inf.getHandles(), inf.getNicho(),
        inf.getAudienciaTotal(), inf.getObservacoes(), Arrays.asList(inf.getTags()),
        inf.getCreatedAt(), inf.getUpdatedAt());
  }
