...
public class MarcaService {
  ...
  public Marca criar(AuthPrincipal principal, MarcaRequest req) {
    ...
    auditLogService.log(
        principal.assessoriaId(), principal.usuarioId(), "marca", marca.getId(),
        AuditLog.Acao.CREATE, toMap(marca));
    return marca;
  }
  ...
  public Marca atualizar(AuthPrincipal principal, UUID id, MarcaRequest req) {
    ...
    marca = repo.save(marca);
    auditLogService.log(
        principal.assessoriaId(), principal.usuarioId(), "marca", id,
        AuditLog.Acao.UPDATE, toMap(marca));
    return marca;
  }
  ...
  private Map<String, Object> toMap(Marca m) {
    return Map.of("id", m.getId().toString(), "nome", m.getNome(),
        "segmento", m.getSegmento() != null ? m.getSegmento() : "",
        "tags", Arrays.asList(m.getTags()));
  }
}
