...
- **DSR endpoints públicos**: `/api/v1/dsr/**` em `permitAll()` (SecurityConfig) — titular acessa sem JWT. Admin endpoints em `/api/v1/admin/compliance/**` requerem `OWNR`. `GET /dsr/dados` exige `?solicitacaoId=UUID` vinculado ao titularId + status EM_ANDAMENTO/CONCLUIDA + tipo ACESSO/PORTABILIDADE
...
- **MFA TOTP secret cifrado**: `mfa_secret_enc` armazena `base64Nonce:base64Ciphertext` (AES-GCM 256-bit via `EmailCipherService`). Chave dedicada `MFA_KEY` / `app.secrets.mfa-key`. Usuários legados com secret em plaintext precisam redo MFA setup
...
- **`/api/v1/whatsapp/webhook` scan por phoneNumberId**: busca todas contas para encontrar a certa pelo `phone_number_id` do evento — pode ser otimizado com índice em tabela quando escalar. Assinatura `X-Hub-Signature-256` é obrigatória (POST sem header → 401). GET verify_token validado contra `app.whatsapp.verify-token` / `WHATSAPP_VERIFY_TOKEN`
